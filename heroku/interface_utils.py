...
def get_rating(rating_meta, target_tone, clf_probs, as_int=False):
    ...
    for prob in clf_probs[0][np.arange(len(clf_probs[0]))!=(target_tone-1)]:
        if prob >= 0.9:
            return rating_meta[-1]["label"] if not as_int else 1

    # if your target probability is more than 50%, you are good
    if target_prob > 0.5:
        return rating_meta[0]["label"] if not as_int else 5
    else:
        return rating_meta[1]["label"] if not as_int else 3
